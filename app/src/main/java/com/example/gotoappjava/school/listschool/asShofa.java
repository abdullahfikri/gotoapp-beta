...
public class asShofa extends ListActivity {
    ...
    private void getPilihan(String pilih) {
        try {
            ...
            switch (pilih){
                case "Call Center":
                    String nomorTelp = "tel:0823-8183-5441";
                    intent = new Intent(Intent.ACTION_DIAL, Uri.parse(nomorTelp));
                    break;
                case "SMS Center":
                    String smsText = "Halo Sekolah As-Shofa, saya ";
                    intent = new Intent(Intent.ACTION_VIEW);
                    intent.setData(Uri.parse("sms:0823-8183-5441"));
                    intent.putExtra("sms_body",smsText);
                    break;
                case "Driving Direction":
                    String lokasi = "google.navigation:q=0.5006, 101.3976";
                    intent = new Intent(Intent.ACTION_VIEW, Uri.parse(lokasi));
                    break;
                case "Website":
                    String linkWeb = "https://asshofa.sch.id/";
                    intent = new Intent(Intent.ACTION_VIEW, Uri.parse(linkWeb));
                    break;
                case "Info Google":
                    intent = new Intent(Intent.ACTION_WEB_SEARCH);
                    intent.putExtra(SearchManager.QUERY, "Sekolah As-Shofa");
                    break;
                case "Exit":
